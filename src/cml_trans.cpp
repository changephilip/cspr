...
//    outbin=fopen("/home/qjq/data/qjq-200-data-posi","wb");
    ...
    for (i=0;i<numItem;i++){
        ...
        cv::Mat lpdft_mrc(afdft_mrc.size(),afdft_mrc.type());
        CML::linearpolar(afdft_mrc,lpdft_mrc);\

//        MrcProcess::showimagecpp(lpdft_mrc);

//        cv::normalize(lpdft_mrc,lpdft_mrc,0,1,CV_MINMAX);
        CML::image_to_mat(lpdft_mrc,tmpdft,dft_size);
        l=0;
        for (j=0;j<dft_size;j++){
            for (k=0;k<dft_size;k++){
                tmpdft[l]=lpdft_mrc.at<float>(j,k);
                l=l+1;
            }

        }
        fwrite(tmpdft,sizeof(tmpdft[0]),dft_size_pow,outbin);
...
//        cv::normalize(lpdft_mrc,lpdft_mrc,0,1,CV_MINMAX);
        ...
        float tmptest[dft_size_pow];
        fread(tmptest,sizeof(tmptest[0]),dft_size_pow,testbin);

        float sum_pow=0.0;
        for ( j=0;j<dft_size_pow;j++){
            sum_pow=sum_pow+(tmptest[j]-tmpdft[j])*(tmptest[j]-tmpdft[j]);
        }
