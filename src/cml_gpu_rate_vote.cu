...
bool comp(const voted &a, const voted &b) { return a.value < b.value; }
...
float cvoting(int cmlij, int cmlik, int cmlji, int cmljk, int cmlki, int cmlkj,
              float cons2) {
  double a, b, c;
...
cmlncv_tuple NCC_value(float *Ci, float *Cj, int after_dft_size) {
  ...
  for (i = 0; i < after_dft_size; i++) {
    ...
    for (j = 0; j < after_dft_size; j++) {
      ...
      //            printf("\n0000002");
      //        value[i][j] =
      //        BNCC(&Ci[i*after_dft_size],&Cj[j*after_dft_size],after_dft_size);
      value[i][j] = 1.0f;
      //            printf("\n000003");
    }
  }
...
cmlncv_tuple NCC_value0(float *Ci, float *Cj, int after_dft_size) {
  ...
  for (i = 0; i < after_dft_size; i++) {
    ...
    for (j = 0; j < after_dft_size; j++) {
      ...
      //            printf("\n0000002");
      //            value[i][j] =
      //            NCC0(&Ci[i*after_dft_size],&Cj[j*after_dft_size],after_dft_size);
      value[i][j] = 1.0f;
      //            printf("\n000003");
    }
  }
...
cmlncv_tuple NCC_Q(float *Ci, float *Cj, int after_dft_size) {
  ...
  //#pragma omp parallel for
  for (i = 0; i < after_dft_size; i++) {
    //        Qci[i][0] = cblas_sasum( after_dft_size, &Ci[i*after_dft_size],
    //        1);//sum
    Qci[i][0] = MYSUM(after_dft_size, &Ci[i * after_dft_size]);
    Qci[i][1] = Qci[i][0] / after_dft_size; // mean
    Qci[i][2] = cblas_sdot(after_dft_size, &Ci[i * after_dft_size], 1,
                           &Ci[i * after_dft_size], 1); // dot
    Qci[i][3] =
        sqrt((Qci[i][2] + after_dft_size * Qci[i][1] * Qci[i][1] -
              2 * Qci[i][0] * Qci[i][1]) /
             after_dft_size); // sigma=sqrt(dot+mean*mean*size-2*mean*sum)
  }
  //#pragma omp parallel for
  for (j = 0; j < after_dft_size; j++) {
    //        Qcj[j][0] = cblas_sasum( after_dft_size, &Cj[j*after_dft_size],
    //        1);//sum
    Qcj[j][0] = MYSUM(after_dft_size, &Cj[j * after_dft_size]);
    Qcj[j][1] = Qcj[j][0] / after_dft_size; // mean
    Qcj[j][2] = cblas_sdot(after_dft_size, &Cj[j * after_dft_size], 1,
                           &Cj[j * after_dft_size], 1); // dot
    Qcj[j][3] =
        sqrt((Qcj[j][2] + after_dft_size * Qcj[j][1] * Qcj[j][1] -
              2 * Qcj[j][0] * Qcj[j][1]) /
             after_dft_size); // sigma=sqrt(dot+mean*mean*size-2*mean*sum)
  }
  ...
  for (i = 0; i < after_dft_size; i++) {
    ...
    for (j = 0; j < after_dft_size; j++) {
      ...
      value[i][j] = (cblas_sdot(after_dft_size, &Ci[i * after_dft_size], 1,
                                &Cj[j * after_dft_size], 1) +
                     after_dft_size * Qci[i][1] * Qcj[j][1] -
                     Qci[i][1] * Qcj[j][0] - Qci[i][0] * Qcj[j][1]) /
                    (after_dft_size * Qci[i][3] * Qcj[j][3]);
    }
  }
  for (i = 0; i < after_dft_size; i++) {
    for (j = 0; j < after_dft_size; j++) {
...
cmlncv_tuple NCC_QT(float **Qci, float **Qcj, float *Ci, float *Cj,
                    int after_dft_size) {
  //  Ci,Cj,two-dem matrix
  //  change to one-d array
  cmlncv_tuple ret;
  ...
  1);//sum
          Qci[i][1] = Qci[i][0] / after_dft_size;//mean
          Qci[i][2] = cblas_sdot( after_dft_size, &Ci[i*after_dft_size],
  1,&Ci[i*after_dft_size],1);//dot
          Qci[i][3] = sqrt((Qci[i][2] + after_dft_size*Qci[i][0]*Qci[i][0] -
  2*Qci[i][0]*Qci[i][1])/after_dft_size);//sigma=sqrt(dot+mean*mean*size-2*mean*sum)
      }
  #pragma omp parallel for
      for (i=0;i<after_dft_size;i++){
          Qcj[i][0] = cblas_sasum( after_dft_size, &Cj[i*after_dft_size],
  1);//sum
          Qcj[i][1] = Qcj[i][0] / after_dft_size;//mean
          Qcj[i][2] = cblas_sdot( after_dft_size, &Cj[i*after_dft_size],1,
  &Cj[i*after_dft_size],1);//dot
          Qcj[i][3] = sqrt((Qci[i][2] + after_dft_size*Qcj[i][0]*Qcj[i][0] -
  2*Qcj[i][0]*Qcj[i][1])/after_dft_size);//sigma=sqrt(dot+mean*mean*size-2*mean*sum)
      }
  */
  //    printf("see Qci\n");
  //    printf("%f",Qci[0][1]);
...
//#pragma omp parallel for
      for(j=0;j<after_dft_size;j++){
          ...
          value[i][j] = (cblas_sdot(after_dft_size,&Ci[i*after_dft_size],1,
&Cj[j*after_dft_size],1
)+after_dft_size*Qci[i][1]*Qcj[j][1]-Qci[i][1]*Qcj[j][0]-Qci[i][0]*Qcj[j][1])/(after_dft_size*Qci[i][3]*Qcj[j][3]);
//            printf("%f",value[i][j]);
      }
  ...
  */
  ...
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, after_dft_size,
              after_dft_size, after_dft_size, 1, Ci, after_dft_size, Cj,
              after_dft_size, 0, C, after_dft_size);

  for (i = 0; i < after_dft_size; i++) {
    //#pragma omp parallel for
    for (j = 0; j < after_dft_size; j++) {
      value[i][j] =
          (C[i * after_dft_size + j] + after_dft_size * Qci[i][1] * Qcj[j][1] -
           Qci[i][1] * Qcj[j][0] - Qci[i][0] * Qcj[j][1]) /
          (after_dft_size * Qci[i][3] * Qcj[j][3]);
    }
  }
...
//__device__ float
/*
__global__ void parent_ncc_kernel(float *d_data,int *d_ctr_id1,int
*d_ctr_id2,float *d_sum,float *d_mean,float *d_stdv,int N,int *Sx,int *Sy,float
*Svalue){
     //????id
     //??cublas?????cublas_sgemm
     //??????C3,??sgemmm???????160K,???????????local memory,???C3???global
memory
     //????????????????????
...
/*
void wrapper_kernel(float *data,int N,int cml_size,float ***help,int *Sx,int
*Sy,float *Svalue){
    //wrapper_kernel????????????????
    //??????
    //?????????????????????cuda?????kernel
    ...
    //printf("N\t%d\n",N);
//    int BLOCK_SIZE;//???????
 //   int THREAD_PER_BLOCK;//(<512,???????cuda???)
    //???????alpha_ij????,ctr?alphaij??
//    ctr = (int *)malloc(control_size);
   // int *ctr;
    ...
    for (i=0;i<N;i++){
        for (j=i+1;j<N;j++){
//an error here ,the id is not eq i;should be modified
later.((2*local_N-1-i)*i/2+j-(i+1))
...
*/
__global__ void simple_max_kernel(float *data, float *d_max_value,
                                  int *d_max_index) {
...
__global__ void my_reduction_kernel1(float *d_in, float *d_out,
                                     int *d_out_index, int N) {
...
__global__ void my_reduction_kernel2(float *d_in, int *index_in, float *d_out,
                                     int *d_out_index, int N) {
...
__global__ void flambda(float *d_process, float *d_sum, float *d_mean,
                        float *d_stdv, int image_a, int image_b) {
  int id = threadIdx.x + blockDim.x * blockIdx.x;
  int i = blockIdx.x;
  int j = threadIdx.x;
  d_process[id] =
      (d_process[id] + L * d_mean[image_a * L + i] * d_mean[image_b * L + j] -
       d_sum[image_a * L + i] * d_mean[image_b * L + j] -
       d_mean[image_a * L + i] * d_sum[image_b * L + j]) /
      (L * d_stdv[image_a * L + i] * d_stdv[image_b * L + j]);
}
/*
__global__ void huge_kernel(int ctrid,int *d_ctr1,int *d_ctr2,float
*d_data,float *d_sum,float *d_mean,float *d_stdv,float *d_buffer,float *d_p,int
*d_pi,float *d_Svalue,int *d_max_index,int N){
...
*/
__global__ void block_kernel(int *d_ctr1, int *d_ctr2, float *d_data,
                             float *d_sum, float *d_mean, float *d_stdv,
                             float *d_Svalue, int *d_max_index) {
  ...
  for (i = 0; i < 16; i++) {
    dot_result[i] =
        (dot_result[i] +
         L * d_mean[image_a * L + index_a[i]] *
             d_mean[image_b * L + index_b[i]] -
         d_sum[image_a * L + index_a[i]] * d_mean[image_b * L + index_b[i]] -
         d_mean[image_a * L + index_a[i]] * d_sum[image_b * L + index_b[i]]) /
        (L * d_stdv[image_a * L + index_a[i]] *
         d_stdv[image_b * L + index_b[i]]);
  }
  ...
  for (int activethreads = 1024 >> 1; activethreads > 32; activethreads >>= 1) {
    if (globalthread < activethreads) {
      sp[globalthread] =
          fmaxf(sp[globalthread], sp[globalthread + activethreads]);
...
__global__ void block_max_kernel(float *d_data, float *d_max_value,
                                 int *d_max_index) {
...
void stream_wrapper_kernel(float *data, int N, int cml_size, float ***help,
  ...
  for (int i = 0; i < b; i++) {
    for (int j = 0; j < a; j++) {
      ...
      my_reduction_kernel1<<<1, L, 0, stream[j]>>>(
          &d_buffer[j * L_power], &d_p[j * L], &d_pi[j * L], L);
      my_reduction_kernel2<<<1, 1, 0, stream[j]>>>(&d_p[j * L], &d_pi[j * L],
                                                   &d_Svalue[a * i + j],
                                                   &d_max_index[a * i + j], L);
    }
    //        cudaDeviceSynchronize();
...
void stream_wrapper_2_kernel(float *data, int N, int cml_size, float ***help,
                             int *Sx, int *Sy, float *Svalue) {
  int c_size = N * (N - 1) / 2;
  int a, b;
  if (N % 2 == 0) {
...
__global__ void flambda_new_kernel(float *d_buffer, float *d_sum, float *d_mean,
...
void batch_gemm_test_wrapper(float *data, int N, int cml_size, float ***help,
  ...
  //**************************************************************************************
  // dont use stream method,use sgemm_batch method to launch a lot of small
  // matrix multiply
  // the function args is like below:
  // MxM * M*N => M*N
  // handle,CUBLAS_OP_N,CUBLAS_OP_T,M,N,N,&alpha,d_Marray,d_Narray,N,&beta,d_Parray,M,num
  // of Matrix
  // d_Marray contain the pointer to device memory
  //**************************************************************************************
  cublasHandle_t handle;
  cublasCreate(&handle);
  const float alpha = 1.0;
...
*/
void block_wrapper_kernel(float *data, int N, int cml_size, float ***help,
                          int *Sx, int *Sy, float *Svalue) {
  int c_size = N * (N - 1) / 2;
  int a, b;
  if (N % 2 == 0) {
  ...
  printf("line 1110\n");
  block_kernel<<<dimGrid, dimBlock, sharedsize>>>(d_ctr_id1, d_ctr_id2, d_data,
                                                  d_sum, d_mean, d_stdv,
...
__device__ float gpu_cvoting(int cmlij, int cmlik, int cmlji, int cmljk,
                             int cmlki, int cmlkj) {
  ...
  if (cmlij == -1 or cmlik == -1 or cmljk == -1) {
    ...
    a = __cosf((cmlkj - cmlki) * gpu_cons2);
    b = __cosf((cmljk - cmlji) * gpu_cons2);
    c = __cosf((cmlik - cmlij) * gpu_cons2);

    if ((1 + 2 * a * b * c) > (a * a + b * b + c * c)) {
      t = (a - b * c) / (sqrt(1 - b * b) * sqrt(1 - c * c));
      if (t < 1.0f and t > -1.0f) {
        angleij = acosf(t) * gpu_cons;
      } else if (t >= 1) {
        angleij = 0.0f;
      } else if (t <= -1) {
        angleij = 180.0f;
...
__global__ void gpu_combined_kernel(int N, int *index_i, int *index_j,
                                    int *CML_Matrix, int T, float *hist_value) {
  ...
  sp[threadIndex] = gpu_cvoting(
      CML_Matrix[i * N + j], CML_Matrix[i * N + threadIndex],
      CML_Matrix[j * N + i], CML_Matrix[j * N + threadIndex],
      CML_Matrix[threadIndex * N + i], CML_Matrix[threadIndex * N + j]);
  sp[threadIndex + 1024] = gpu_cvoting(
      CML_Matrix[i * N + j], CML_Matrix[i * N + (threadIndex + 1024)],
      CML_Matrix[j * N + i], CML_Matrix[j * N + (threadIndex + 1024)],
      CML_Matrix[(threadIndex + 1024) * N + i],
      CML_Matrix[(threadIndex + 1024) * N + j]);
  sp[threadIndex + 2048] = gpu_cvoting(
      CML_Matrix[i * N + j], CML_Matrix[i * N + (threadIndex + 2048)],
      CML_Matrix[j * N + i], CML_Matrix[j * N + (threadIndex + 2048)],
      CML_Matrix[(threadIndex + 2048) * N + i],
      CML_Matrix[(threadIndex + 2048) * N + j]);
  ;
  sp[threadIndex + 3072] = gpu_cvoting(
      CML_Matrix[i * N + j], CML_Matrix[i * N + (threadIndex + 3072)],
      CML_Matrix[j * N + i], CML_Matrix[j * N + (threadIndex + 3072)],
      CML_Matrix[(threadIndex + 3072) * N + i],
      CML_Matrix[(threadIndex + 3072) * N + j]);
  ;
  if (threadIndex < 904) {
    sp[threadIndex + 4096] = gpu_cvoting(
        CML_Matrix[i * N + j], CML_Matrix[i * N + (threadIndex + 4096)],
        CML_Matrix[j * N + i], CML_Matrix[j * N + (threadIndex + 4096)],
        CML_Matrix[(threadIndex + 4096) * N + i],
        CML_Matrix[(threadIndex + 4096) * N + j]);
    ;
  }
  __syncthreads();
  for (int s = 0; s < T; s++) {
    ...
    if (sp[threadIndex] != -10.0) {
      alpha_t_alpha12 = gpu_Trecurse * s - sp[threadIndex];
      sp_tmp[threadIndex] =
          exp(-1.0 * alpha_t_alpha12 * alpha_t_alpha12 / gpu_two_sigma_pow) /
          gpu_half_pow_pi;
    } else {
      sp_tmp[threadIndex] = 0.0f;
    }
    if (sp[threadIndex + 1024] != -10.0) {
      alpha_t_alpha12 = gpu_Trecurse * s - sp[threadIndex + 1024];
      sp_tmp[threadIndex + 1024] =
          exp(-1.0 * alpha_t_alpha12 * alpha_t_alpha12 / gpu_two_sigma_pow) /
          gpu_half_pow_pi;
    } else {
      sp_tmp[threadIndex + 1024] = 0.0f;
    }
    if (sp[threadIndex + 2048] != -10.0) {
      alpha_t_alpha12 = gpu_Trecurse * s - sp[threadIndex + 2048];
      sp_tmp[threadIndex + 2048] =
          exp(-1.0 * alpha_t_alpha12 * alpha_t_alpha12 / gpu_two_sigma_pow) /
          gpu_half_pow_pi;
    } else {
      sp_tmp[threadIndex + 2048] = 0.0f;
    }
    if (sp[threadIndex + 3072] != -10.0) {
      alpha_t_alpha12 = gpu_Trecurse * s - sp[threadIndex + 3072];
      sp_tmp[threadIndex + 3072] =
          exp(-1.0 * alpha_t_alpha12 * alpha_t_alpha12 / gpu_two_sigma_pow) /
          gpu_half_pow_pi;
    } else {
      sp_tmp[threadIndex + 3072] = 0.0f;
    }
    //        if (sp[threadIndex + 4096] != -10.0f) {
    //            alpha_t_alpha12 = Trecurse * s - sp[threadIndex + 4096];
    //            sp_tmp[threadIndex + 4096] = exp(-1.0 * alpha_t_alpha12 *
    //            alpha_t_alpha12 / two_sigma_pow) / half_pow_pi;
    ...
    if (threadIndex < 1024) {
      sp_tmp[threadIndex] += sp_tmp[threadIndex + 1024] +
                             sp_tmp[threadIndex + 2048] +
                             sp_tmp[threadIndex + 3072];
      if (threadIndex < 904) {
    ...
    __syncthreads();
    for (int activethreads = 1024 >> 1; activethreads > 32;
         activethreads >>= 1) {
  ...
  if (threadIndex < 8) {
    tmp_hist[threadIndex] =
        fmaxf(tmp_hist[threadIndex], tmp_hist[threadIndex + 64]);
  }
  __syncthreads();
...
void Voting_wrapper(int **CML_Matrix, int N, float *hist_return, int T,
                    int dft_size) {
  ...
  cudaMemcpy(d_cml_maxtrix, cml_oned_matrix, sizeof(int) * c_size,
  ...
  // allocate constant
  float cs_gpu_cons = 180.0 / M_PI;
  float cs_gpu_cons2 = 2 * M_PI / dft_size;
  float cs_gpu_trecurse = 180.0f / T;
  ...
  gpu_combined_kernel<<<dimGrid, dimBlock>>>(N, d_index_i, d_index_j,
                                             d_cml_maxtrix, T, d_hist_return);
  cudaDeviceSynchronize();

  cudaMemcpy(hist_return, d_hist_return, sizeof(float) * c_size,
             cudaMemcpyDeviceToHost);
...
void List_wrapper(int *inList, FILE *f, FILE *log, FILE *particle_log,
                  int dft_size, int dft_size_pow, FILE *debug_f, FILE *hist_f,
                  std::vector<voted> &good_Particle) {
  ...
  for (int i = 0; i < local_N; i++) {
    for (int j = 0; j < dft_size; j++) {
      ...
      pre_Ncc[i][j][2] = cblas_sdot(dft_size, &data_Matrix[postion], 1,
                                    &data_Matrix[postion], 1); // dot
      pre_Ncc[i][j][3] = sqrt(
          (pre_Ncc[i][j][2] + dft_size * pre_Ncc[i][j][1] * pre_Ncc[i][j][1] -
           2 * pre_Ncc[i][j][0] * pre_Ncc[i][j][1]) /
          dft_size); // sigma=sqrt(dot+mean*mean*size-2*mean*sum)
    }
  }
  // give work to gpu
  int *Sx;
        ...
        for (int k = 0; k < local_N; k++) {
          if (k != i and k != j) {
            if (cml_Pair_Matrix[i][k] != -1 and cml_Pair_Matrix[j][k] != -1) {
              tmp_Voting[k] =
                  cvoting(cml_Pair_Matrix[i][j], cml_Pair_Matrix[i][k],
                          cml_Pair_Matrix[j][i], cml_Pair_Matrix[j][k],
                          cml_Pair_Matrix[k][i], cml_Pair_Matrix[k][j], cons);
            } else {
              tmp_Voting[k] = -10.0;
            }
          } else {
...
#pragma omp parallel for
  ...
  fprintf(log, "gpu_voting_time %d\n", time_gpu_voting - time_Voting);
  fprintf(log, "Sort_Time %d\n", time_Sort - time_gpu_voting);
}
...
int main(int argc, char *argv[]) {
  ...
  while ((oc = getopt(argc, argv, "s:f:i:l:o:d:n:h")) != -1) {
    ...
    case 'n':
      ...
      break;
    case 'h':
      printf("CML_NONPART is a program that picks high quality particles and "
             "throw out non-particles\n");
      printf("Author:\tQian JiaQiang ,Fudan "
             "Univesity,15210700078@fudan.edu.cn\tHuangQiang Lab\n");
      printf("-s the particle size after dft and linear polar,default is "
             "128(only 128 supported\n");
      printf("-n the number of particles\n");
      printf("-f the mrcs file which contains the particles data\n");
      printf("-l your output filename,which will contain the particles,-o your "
             "output log file\n");
      printf("example\n");
      printf("cml_noise -s 128 -f ~/data/data.mrc -i 5 -l ~/output/particles "
             "-o ~/output/log\n");
      exit(EXIT_SUCCESS);
      break;
    }
  ...
  //        printf("-n N and -s dft_cml_size are both needed,if N=0,then
  //        N=max\n");
  //        exit(EXIT_FAILURE);
  //    }
  if (!logfile) {
...
*/
  ...
  /*
    for (int
control_iteration=iteration;control_iteration>0;control_iteration=control_iteration-1){
        //??????5000*n??
  ...
  for (int control_iteration = iteration; control_iteration > 0;
    ...
    for (int x : Global_Particle) {
      ...
      iter=std::find(Global_Good_Particle.begin(),Global_Good_Particle.end(),x);
      if (iter==Global_Good_Particle.end()){
          Particle_Current.push_back(x);
      }
      */
      std::vector<int>::iterator iter =
          std::find(Global_Bad_Particle.begin(), Global_Bad_Particle.end(), x);
    ...
    for (int child = 0; child < Current_Align_P / local_N; child = child + 1) {
      List_wrapper(&List_Current[child * local_N], f, OUTFILE, outputfile,
                   dft_size, dft_size_pow, debugfile, histfile, this_turn);
